    ...
    <div>
        ...
        <el-container>
            ...
            <el-main id="elMain">
                <!-- logo -->
                <div id="logo">
                    <el-image src="imgs/login/logo.png" id="logoImg" :fit="'scale-down'" alt="Logo"></el-image>
                </div>
                <!-- info -->
                <div id="info">
                    <!-- 介绍 -->
                ...
                <div id="form">
                    <el-form ref="form" :model="form" label-width="80px">
                      ...
                      <el-form-item> <!-- 验证码 -->
                        <el-radio v-for="item in roles" v-bind:key="item.type" v-model="form.type" :label="item.type">{{item.roleName}}</el-radio>
                      </el-form-item>
...
<script>
    ...
    export default {
        ...
        data(){
            return{
                roles: [{"roleName": "管理员", "type": "0"},{"roleName": "用户", "type": "1"} ],
                // 动画控制器
                show: true,
                // 表单数据模型
                form:{
            ...
            onSubmit(){
                ...
                this.$axios.post("/LoginCon/login",this.form).then((res)=>{
                    if(res.data.flag){
                        ...
                        if(this.form.type === '1') // 用户登录
                        {
                            this.$router.push({
                                name:'UserMainPage',
                            })
                        }
                        else if (this.form.type === '0')// 管理员登录
                        {
                            this.$router.push({
                                name: 'AdminMainPage',
                            })
