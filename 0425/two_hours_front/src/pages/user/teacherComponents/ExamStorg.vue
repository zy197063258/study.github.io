            ...
            <el-col :span="7">
                <el-row>
                    <el-col :span="22" :offset="1">
                        <el-button type="primary" plain @click="createExam">新增考试题库</el-button>
                    </el-col>
                </el-row>
            </el-col>
            <!--搜索表单-->
    ...
    export default {
        ...
        methods:{
            ...
            getExam(){
                ...
                this.$axios.post("/Store/getExam",this.searchTitle).then((res)=>{
                    if(res.data.flag){
                        /* 保存公告数据 */
            ...
            submitExam(exam){
                ...
                this.$refs['createExamForm'].validate((valid) => {
                    if (valid && this.editExamData.dateRange !== null) {
                        ...
                        if(this.isEditStatue){
                            // 编辑表单状态，通过examId更新数据
                            this.$axios.post("/Store/updateExam",exam).then((res)=>{
                                if(res.data.flag){
                            ...
                            exam.id = undefined;
                            // 创建表单状态，创建一条新数据
                            this.$axios.post("/Store/createExam",exam).then((res)=>{
                                if(res.data.flag){
                                    this.$message.success(res.data.msg);
                                    // 关闭窗口
                                    this.showEditExam = false;
            ...
            // 重置表单
            resetFormData(){
                this.editExamData.id = undefined;
            ...
            updateExamState(exam){
                ...
                this.$axios.post("/Store/updateInfo",newExam).then((res)=>{
                    if(res.data.flag){
                        // 修改成功
                        this.$message.success(res.data.msg);
                    }else{
            ...
            deleteExam(row){
                ...
                this.$confirm('此操作将删除该考试及相关数据, 是否继续?', '提示', {
                    ...
                    let exam = {id:row.id};
                    // 执行删除
                    this.$axios.post("/Store/deleteExamById",exam).then(resp=>{
                        if(resp.data.flag){
                            // 删除成功，刷新数据
                            this.getExam();
