...
<script>
    ...
    export default {
        ...
        methods:{
            // 查询考试数据
            getExam(){
                let exam = {id:this.$route.params.examId};
                this.$axios.post("/Store/getExamById", exam).then((res)=>{
                    if(res.data.flag){
                        /* 保存公告数据 */
            ...
            submitSubject(subjectOrg){
                ...
                // 封装数据
                let subjectInfo = {subject:subjectOrg,examId:this.exam.id};
                // 提交状态
