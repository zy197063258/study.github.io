...
public class CourseUserCon {
    ...
    @RequestMapping("/updateAddResDlNum")
    @PostMapping
    public DEA updateInfo( HttpServletRequest request) {
        String userId = (String) request.getSession().getAttribute("userId");
        Integer courseId = (Integer) request.getSession().getAttribute("courseId");
        QueryWrapper<CourseUser> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq("course_id",courseId);
        queryWrapper.eq("user_id",userId);
        CourseUser courseUserServiceOne = courseUserService.getOne(queryWrapper);
        courseUserServiceOne.setResDlNum(courseUserServiceOne.getResDlNum()+1);
        // 更新数据
        return new DEA(courseUserService.updateById(courseUserServiceOne));
    }
