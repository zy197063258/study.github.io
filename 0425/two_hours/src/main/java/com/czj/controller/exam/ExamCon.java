...
public class ExamCon {
    ...
    public DEA getExam(@RequestBody SearchForm searchForm, HttpServletRequest request){
        ...
        IPage<Exam> examIPage = examService.page(page, queryWrapper);
        if (CollectionUtils.isNotEmpty(examIPage.getRecords())){
            examIPage.getRecords().forEach(a->{
                QueryWrapper<Store> estoreQueryWrapper = new QueryWrapper<>();
                estoreQueryWrapper.eq("id",a.getStore());
                Store one = storeService.getOne(estoreQueryWrapper);
                a.setStoreName(one.getTitle());
            });
        }
        return  new DEA(examIPage);
    }
